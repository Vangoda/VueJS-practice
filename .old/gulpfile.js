...
async function browserSync() {
  bSync.init({
    open: false,
    injectChanges: true,
    proxy: "https://course.vuejs",
    port: 8001,
    https: {
      key: "/var/www/html/ssl/course.vuejs.key",
      cert: "/var/www/html/ssl/course.vuejs.crt",
    },
  });
}
...
function scss() {
  ...
  return src(styleSrc) // Read the files from src folder
    ...
    .pipe(
      sass({
        // Compile sass, specify extra opions
    ...
    .pipe(
      autoprefixer({
        // Autoprefixer, fixes css compatibility for varius browsers
        cascade: false,
      })
    )
